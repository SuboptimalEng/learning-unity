...
public class TerrainFace
{
    ...
    public void ConstructMesh()
    {
        ...
        Vector2[] uv =
            (mesh.uv.Length == verticies.Length) ? mesh.uv : new Vector2[verticies.Length];
        for (int y = 0; y < resolution; y++)
        {
            for (int x = 0; x < resolution; x++)
            {
                ...
                Vector3 pointOnUnitCube =
                    localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;
                Vector3 pointOnUnitSphere = pointOnUnitCube.normalized;
                float unscaledElevation = shapeGenerator.CalculateUnscaledElevation(
                    pointOnUnitSphere
                );
                verticies[i] =
